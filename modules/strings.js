...
const stringEcnrypt = (a, b) => {
    ...
    for (let i = 0; i < a.length; i++)
        s.push((a.charCodeAt(i) ^ b.charCodeAt((i + 1) % b.length)) - 10);
    return s.join(",");
};
...
exports.protect_strings = string => {
    ...
    const var_name = "es";
    const xor_key = "1234567890";
    const decryptor_lua = `
for c = 1, #${var_name} do
    local array, result, key = ${var_name}[c], "", "${xor_key}" .. tostring(c)
    local op, op2 = string.reverse("roxb"), string.reverse("rahc")
    for i = 1, #array do
        result = result .. string[op2](bit[op](array[i] + 10, key:sub(i % #key + 1, i % #key + 1):byte()))
    end
    ${var_name}[c] = result
end`;
    do {
