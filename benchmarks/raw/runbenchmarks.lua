...
-- List of tests
local tests_root = io.popen("cd"):read('*l') .. "/"
local tests = {
    { 'ack', 'ack.lua 3 10' },
...
local function measure(cmd)
    local time_cmd = 'powershell -Command "Measure-Command {'..cmd..'}  | select -expand TotalSeconds"'
...
local function create_data_file(results)
    ...
    local file = io.open("./" .. basename .. '.txt', 'w')
    if not file then
        error('Could not create file "' .. basename .. '.txt"')
    end
    file:write(data)
    file:close()
end
...
local function setup()
    io.popen('("../../luajit" fasta.lua 1000000) > fasta1000000.txt'):close()
end

local function teardown()
    os.remove('fasta1000000.txt')
end
