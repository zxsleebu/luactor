...
exports.protect_literals = string => {
    ...
    luaparse.parse(string, {
        onCreateNode: async s => (s.type == "StringLiteral") && parse_literal(s),
        ranges: true,
    })
    luaparse.parse(string, {
        onCreateNode: async s => (s.type == "NumericLiteral") && parse_literal(s),
        ranges: true,
    })
    luaparse.parse(string, {
        onCreateNode: async s => (["NilLiteral", "BooleanLiteral"].includes(s.type)) && parse_literal(s),
        ranges: true,
    })
    var literals = [];
    ...
    for(let type of ["StringLiteral", "NumericLiteral"]){
        ...
        for(let value in literal_obj[type]){
            ...
            encrypted[type][literal.index] = stringEcnrypt(value, xor_key + literal.index);
            add_literal(literal, value)
            process.stdout.cursorTo(0);
            process.stdout.write(`Encrypted ${++encrypted_count} literals!`);
        }
    ...
    literals.forEach(l => {
        process.stdout.cursorTo(0);
        process.stdout.write(`Protected ${++protected_count} literals!`);
